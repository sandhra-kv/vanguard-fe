...
const ListItems = ({ data, header, onSelect }) => {
         ...
         <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="divide-y  bg-white">
               {sortedData.map((field) => (
                  <tr key={field.id} className=" text-sm cursor-pointer" onClick={() => onSelect(field)}>
                     {/* <td className="pl-6 py-5 w-0.5">
                        <input type="checkbox" />
                     </td> */}
                     <td className="px-6 py-5 flex items-center gap-2">
                        <img src="/images/lead.png" className="size-10" alt="avatar" />
                        {field.name}
                     </td>
                     ...
                     <td className="py-3 pr-6">
                        <button className="border px-3 py-2 rounded-lg">
                           <img src={actionButton} alt="edit or delete" />
                        </button>
                     </td>
                  </tr>
               ))}
